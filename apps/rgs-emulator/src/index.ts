...
import { BASE_VORTEX_CHANCE, genBook } from './game.ts';
...
// 4757 is this game's port — distinct from the common 4747 so this emulator
// never collides with a sibling project's emulator on the same machine.
const PORT = Number(process.env.RGS_PORT ?? 4757);
...
const GAME_ID = 'vortex_digits';
/** Outcomes are generated procedurally (see game.ts), so RTP is illustrative. */
const RTP_PERCENT = Number(process.env.RGS_RTP ?? 96);
/** The bet modes the emulator serves: base + the paid vortex ante. */
const MODES = new Set(['base', 'vortex']);
...
function handlePlay(sessionID: string, amount: number, modeName: string): PlayResponse {
  const s = getSession(sessionID);
...
const server = createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && url === '/health') {
    return send(res, 200, { ok: true, gameID: GAME_ID, rtp: RTP_PERCENT / 100, baseVortexChance: BASE_VORTEX_CHANCE });
  }
  ...
  if (req.method === 'GET' && url.startsWith('/bet/replay/')) {
    ...
    const book = genBook(modeName);
    return send(res, 200, { payoutMultiplier: book.payoutMultiplier, costMultiplier: 1, state: book.events });
  }
  ...
  try {
    ...
    switch (url.split('?')[0]) {
      ...
      case '/wallet/play':
        return send(res, 200, handlePlay(sessionID, Number(body.amount), String(body.mode ?? 'base')));
...
server.listen(PORT, () => {
  ...
  console.log(`[rgs] procedural outcomes — base vortex chance ${(BASE_VORTEX_CHANCE * 100).toFixed(0)}%, modes: ${[...MODES].join(', ')}`);
