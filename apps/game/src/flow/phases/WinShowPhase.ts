...
export class WinShowPhase implements Phase {
  ...
  async enter(ctx: PhaseContext): Promise<void> {
    ...
    const isWin = winlines.length > 0 && totalWin > 0;
    ...
    if (isWin) {
      ...
      // Big-win threshold: 20× the bet earns the celebratory arpeggio.
      sfx.play(totalWin >= ctx.stores.balance.bet * 20 ? 'bigWin' : 'win');
    }

    if (isWin) ctx.reels.showWin(winlines);
    const hold = isWin ? WIN_HOLD_MS : AUTOSPIN_DELAY_MS;
