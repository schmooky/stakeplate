...
import type { Grid, GridCell, Winline } from '@/domain/types';
...
export interface ReelsEngine {
  ...
  forceStop(): void;
  spotlight(cells: GridCell[]): void;
  clearSpotlight(): void;
  /** Swirl a full column of digits into existence (the Vortex feature). */
  vortexSpawn(col: number): Promise<void>;
  dispose(): void;
}
...
export class ReelsPresenter implements Disposable {
  ...
  async vortexSpawn(col: number): Promise<void> {
    await this.engine.vortexSpawn(col);
  }
